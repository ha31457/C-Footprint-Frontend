...
export default function OrgAdminAnalytics() {
  ...
  useEffect(() => {
    const fetchAnalysis = async () => {
      ...
      setError('');
      try {
        const res = await apiClient.get('/org-admin/analysis');
        setData(res.data);
      } catch (err) {
        setError(err.response?.data?.message || 'Failed to fetch organization metrics.');
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  if (loading) return <div className="loading-screen">Loading Organization Metrics...</div>;
  ...
  return (
    <div className="dashboard" style={{ maxWidth: '1280px' }}>
      <header className="dashboard-header" style={{ marginBottom: '2rem' }}>
        <div>
          <h1>Organization Diagnostics & Analytics</h1>
          <p style={{ color: 'var(--text-secondary)', marginTop: '0.2rem' }}>
            Organization-wide activity distributions, average carbon ratios, and employee footprint insights
          </p>
        </div>
      </header>
      ...
      <section className="admin-stats-grid" style={{ marginBottom: '2rem', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>
        <div className="chart-card stat-card" style={{ padding: '1.2rem 1.8rem' }}>
          <span className="stat-label" style={{ fontSize: '0.75rem' }}>Active Employees</span>
        ...
        <div className="chart-card stat-card" style={{ padding: '1.2rem 1.8rem' }}>
          <span className="stat-label" style={{ fontSize: '0.75rem' }}>Avg Emission / Employee</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: '2rem', marginBottom: '2rem' }}>
        ...
        <div className="chart-card" style={{ padding: '2rem' }}>
          <h3 style={{ marginBottom: '1.5rem', fontSize: '1.15rem' }}>Organization Log Volume Share</h3>
        ...
        {/* Category CO2 Share */}
        <div className="chart-card" style={{ padding: '2rem' }}>
          <h3 style={{ marginBottom: '1.5rem', fontSize: '1.15rem' }}>Total Employee Emissions (kg CO₂e)</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                ...
                return (
                  <div key={cat} style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                    ...
                    <div style={{ height: '8px', background: 'var(--border-color)', borderRadius: '9999px', overflow: 'hidden' }}>
                      <div style={{ width: `${pct}%`, height: '100%', background: 'var(--primary-color)', borderRadius: '9999px' }}></div>
                    </div>
                  </div>
                );
              })
      ...
      {safeTips.length > 0 && (
        <section className="chart-card" style={{ padding: '2rem', borderLeft: '4px solid var(--accent-color)' }}>
          <h3 style={{ marginBottom: '1.2rem', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '1.15rem', color: 'var(--accent-color)' }}>
            <span>📢</span> Organization Campaign Recommendations
          </h3>
