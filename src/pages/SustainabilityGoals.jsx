...
export default function SustainabilityGoals() {
  ...
  // Calculate Budget Consumed Percentage (0% for 0 current emission)
  const currentEm = activeGoal?.currentEmission ?? 0;
  const targetEm = activeGoal?.targetEmission ?? 1;
    ...
    <div className="dashboard" style={{ maxWidth: '1280px' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <section className="chart-card" style={{ padding: '2.2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.2rem' }}>
              <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '1.2rem', margin: 0 }}>
                <span>🎯</span> Active Reduction Target
              </h3>
              {activeGoal && (
                <span className="status-badge status-active" style={{ fontSize: '0.85rem', padding: '0.35rem 0.9rem', fontWeight: '800' }}>
                  {activeGoal.periodType} GOAL
                </span>
              )}
            </div>
            ...
            {activeGoal ? (
              <div>
                ...
                {activeGoal.alertMessage && (
                  <div
                    style={{
                      padding: '1.2rem 1.5rem',
                      borderRadius: '14px',
                      marginBottom: '1.8rem',
                      background: activeGoal.isOnTrack ? 'var(--primary-light)' : 'rgba(239, 68, 68, 0.1)',
                      border: `1.5px solid ${activeGoal.isOnTrack ? 'var(--primary-color)' : 'var(--error-color)'}`,
                      color: activeGoal.isOnTrack ? 'var(--primary-color)' : 'var(--error-color)',
                      fontWeight: '750',
                      fontSize: '0.95rem',
                      lineHeight: '1.4'
                    }}
                ...
                {/* Metrics Grid */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1.2rem', marginBottom: '1.8rem' }}>
                  <div style={{ padding: '1.2rem', background: 'var(--bg-color)', borderRadius: '14px', border: '1px solid var(--border-color)' }}>
                    ...
                    <span style={{ fontSize: '1.7rem', fontWeight: '850', color: 'var(--primary-color)' }}>{activeGoal.targetReductionPercentage}%</span>
                  </div>
                  <div style={{ padding: '1.2rem', background: 'var(--bg-color)', borderRadius: '14px', border: '1px solid var(--border-color)' }}>
                    ...
                    <span style={{ fontSize: '1.7rem', fontWeight: '850' }}>{(activeGoal.baselineEmission ?? 0).toFixed(1)} <small style={{ fontSize: '0.85rem' }}>kg</small></span>
                  </div>
                  <div style={{ padding: '1.2rem', background: 'var(--bg-color)', borderRadius: '14px', border: '1px solid var(--border-color)' }}>
                    ...
                    <span style={{ fontSize: '1.7rem', fontWeight: '850', color: 'var(--accent-color)' }}>{(activeGoal.targetEmission ?? 0).toFixed(1)} <small style={{ fontSize: '0.85rem' }}>kg</small></span>
                  </div>
                  <div style={{ padding: '1.2rem', background: 'var(--bg-color)', borderRadius: '14px', border: '1px solid var(--border-color)' }}>
                    ...
                    <span style={{ fontSize: '1.7rem', fontWeight: '850', color: activeGoal.isOnTrack ? 'var(--text-primary)' : 'var(--error-color)' }}>
                      {(activeGoal.currentEmission ?? 0).toFixed(1)} <small style={{ fontSize: '0.85rem' }}>kg</small>
                    </span>
                  </div>
                </div>
                ...
                <div style={{ marginBottom: '1.8rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.88rem', fontWeight: '750', marginBottom: '0.4rem' }}>
                    <span>Emission Budget Consumed</span>
                    <span>{budgetConsumedPct.toFixed(1)}% Used</span>
                  </div>
                  <div style={{ height: '12px', background: 'var(--border-color)', borderRadius: '999px', overflow: 'hidden' }}>
                    <div
                      style={{
                        width: `${budgetConsumedPct}%`,
                        height: '100%',
                        background: activeGoal.isOnTrack
                  ...
                  <span style={{ fontSize: '0.78rem', color: 'var(--text-light)', marginTop: '0.30rem', display: 'block' }}>
                    {(activeGoal.currentEmission ?? 0).toFixed(1)} kg emitted out of {(activeGoal.targetEmission ?? 0).toFixed(1)} kg maximum budget
                  </span>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border-color)', paddingTop: '1.2rem' }}>
                  <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                    📅 Active Duration: <strong>{activeGoal.startDate}</strong> to <strong>{activeGoal.endDate}</strong>
                  </span>
                  <button
                    onClick={() => {
                      setCancellingGoalId(activeGoal.id);
              ...
              <div style={{ textAlign: 'center', padding: '1.2rem 0', color: 'var(--text-secondary)' }}>
                <p style={{ margin: '0 0 0.4rem 0', fontWeight: '700', fontSize: '1rem' }}>No active reduction goal set.</p>
          ...
          {/* Premium Goal Creation Form (Always Available) */}
          <section className="chart-card" style={{ padding: '2.2rem' }}>
            <div style={{ marginBottom: '1.5rem' }}>
              <h3 style={{ fontSize: '1.25rem', marginBottom: '0.3rem' }}>
                {activeGoal ? 'Set New / Replace Active Goal' : 'Activate Reduction Goal'}
              </h3>
              <p style={{ fontSize: '0.88rem', color: 'var(--text-secondary)', margin: 0 }}>
                {activeGoal
                  ? 'Activating a new goal will overwrite your currently active target.'
                  : 'Choose a target reduction percentage and timeline period below.'}
              </p>
            </div>
            ...
            <form onSubmit={handleCreateGoal} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
              ...
              >
                {submitting ? 'Activating Goal...' : activeGoal ? 'Overwrite Active Goal' : 'Activate Goal'}
              </button>
            </form>
          </section>
                  ...
                  <tbody>
                    {goalHistory.map((goal) => (
                      <tr key={goal.id}>
                        ...
                        <td>
                          <span
                            className={`status-badge ${goal.status === 'ACTIVE'
                                ? 'status-active'
      ...
      {showCancelModal && (
        <div className="custom-modal-overlay">
          <div className="custom-modal">
            ...
            <p>Are you sure you want to cancel your active sustainability reduction target? This action cannot be undone.</p>
